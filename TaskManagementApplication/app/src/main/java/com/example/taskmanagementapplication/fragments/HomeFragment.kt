...
import com.example.taskmanagementapplication.adapter.TaskAdaptor
import com.example.taskmanagementapplication.databinding.FragmentHomeBinding
...
class HomeFragment : Fragment(R.layout.fragment_home), SearchView.OnQueryTextListener, MenuProvider {
    ...
    private fun setupHomeRecyclerView(){
        ...
        activity?.let {
            taskViewModel.getAllTasks().observe(viewLifecycleOwner){ task ->
                taskAdaptor.differ.submitList(task)
                updateUI(task)
            }
        }
    }
    ...
    private fun searchTask(query: String?){
        ...
        taskViewModel.searchTasks(searchQuery).observe(this){list ->
            taskAdaptor.differ.submitList(list)
        }
    }
